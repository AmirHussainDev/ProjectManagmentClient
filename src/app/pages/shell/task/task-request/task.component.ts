...
import { TaskStateNames, TaskStates, RestrictNavigationMessage, TaskTypeColors, TaskTypes, TaskStatesConnectivity, TaskSeverity, TaskSeverityColors, AppPermissions } from '../../../../services/app.constants';
import { PdfGeneratorService } from '../../../../services/pdf-generator.service';
import { DataUrl, NgxImageCompressService, UploadResponse } from 'ngx-image-compress';
import { CanComponentDeactivate } from '../../../../guards/can-deactivate.guard';
import { AngularEditorConfig } from '@kylepairish/angular-editor';
@Component({
  selector: 'app-task',
...
getItemFormGroup(object = this.defaultItemValues) {
  ...
  async submitRequest() {
  const response: any = await this.appService.AddTask({
    details: {
...
isSelectedEmployeeHourlyBased(){
const assigneeId=this.taskDetails.get('assignee')?.value
return this.listOfData.some(assignee=>assignee.isHourlyRateHourly&&assignee.employee===assigneeId);
}
...
disableAndEnableSpecificControls() {
  ...
  if (stateControl && stateControl.value !== this.TaskStates.Backlog) {
    ...
    Object.keys(this.taskDetails.controls).forEach(controlName => {
      ...
      if (control &&
          !this.userService.hasPermission(this.appPermissions.AddTask.toString())
          && !this.userService.hasPermission(this.appPermissions.AddTask.toString())
      ) {
